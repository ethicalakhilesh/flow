...
import { getTransaction, getAccounts } from "@/lib/airtableData";
...
export default async function EditTransaction({ params }: { params: { id: string } }) {
  ...
  const [transaction, accounts] = await Promise.all([getTransaction(params.id), getAccounts()]);
    ...
    <AppShell username={username}>
      <div className="flex flex-col gap-4">
        ...
        <form action={updateWithId} className="flex flex-col gap-4">
          <TransactionFormFields defaults={transaction} accounts={accounts} />
          <button
            type="submit"
